...
function calculateFoodOrder(numAnimals, avgFood) {
    ...
    if(Number.isNaN(numAnimals) || Number.isNaN(avgFood)) {
      ...
      totalWeeklyFood = numAnimals * avgFood;
      return totalWeeklyFood;
    }
...
function mostPopularDays(week) {
    ...
    // Sort so most popular day is first in array
    week.sort(function(a,b){
      return b.traffic - a.traffic;
    })

    var weekArrLength = week.length;
    var popularDays = [];
    for(var i = 0; i < weekArrLength; i++){
 ...
 */
 function createAnimalObjects(names, types, breeds) {
     ...
     for(var i = 0; i < arrLength; i++){
       animalObjArray.push(new Animal(names[i],types[i],breeds[i]));
     }
